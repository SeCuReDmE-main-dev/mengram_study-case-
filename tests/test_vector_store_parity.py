...
Verifies that SQLiteVectorStore and SQLiteVecVectorStore conform to BaseVectorStore ABC.
All tests run against both sqlite backends.
"""
...
def raw_backend(request):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        # Ensure optional backends are registered via lazy loading
        if name not in VectorStoreFactory._backends:
            try:
                VectorStoreFactory._load_optional_backend(name)
            except ImportError:
                pytest.skip(f"Backend '{name}' not available")
        # VectorStoreFactory.create() returns the VectorStore facade; we want
        # the raw backend to test the ABC contract directly.
        backend_class = VectorStoreFactory._backends[name]
        kwargs = {"dimension": _DIM}
        if name in ("sqlite", "sqlite_vec"):
